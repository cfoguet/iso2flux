...
if __name__ == "__main__":
     ...
     for x in range(0,p_dict["annealing_iterations"]):
         ...
         parameters,best_flux_dict,f=annealing(label_model,max_random_sample=max_random_sample,min_random_sample=min_random_sample,n=p_dict["annealing_n"],m=p_dict["annealing_m"],p0=p_dict["annealing_p0"],pf=p_dict["annealing_pf"],parameter_precision=label_model.parameter_precision,max_perturbation=p_dict["annealing_max_perturbation"],min_perturbation=p_dict["annealing_min_perturbation"],      fraction_of_optimum=fraction_of_optimum,parameter_dict=parameters,n_processes=p_dict["annealing_n_processes"],output=False)
         if f<f_best:
            f_best=f
