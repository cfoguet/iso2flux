...
class GUI:
      ...
      def add_search_bar(self):
          search_labelframe=LabelFrame(self.root,text="Search bar")
          search_labelframe.pack(side=TOP,expand=False)
          self.search_entry=Entry(search_labelframe)
          self.search_entry.pack(side=TOP,expand=False)
      ...
      def create_gim3e_window(self):
          ...
          button2 = Button(top, text="Constrain using gene expression",command=self. gim3e)
          button2.pack( side = TOP)  
      ...
      def limit_max_fluxes(self):
           ...
           add_flux_limit_constraints(self.label_model.constrained_model,fraction_of_optimum_objective=self.fraction_of_optimum, fraction_of_flux_minimum=fraction_of_flux_minimum,solver=solver)
      ...
      def sampling(self):
          ...
          sampling(self.label_model,n=n,fraction_of_optimum=self.fraction_of_optimum,output_emu_list=None,max_turnover=1000,excluded_turnovers=self.modified_turnovers,fba_mode=self.fba_mode.get(),parameter_precision= self.change_threshold/100.0,gui=self,change_threshold=self.change_threshold,parameter_dict=label_model.parameter_dict)
      ...
      def calculate_confidence_intervals(self):
          ...
          if len(parameter_list)>0:
             fname = tkFileDialog.asksaveasfilename(parent=self.root,title="Save results as...",filetypes=[("xlsx",".xlsx")])
      ...
      def load_model(self):
          new_label_model=load_iso2flux_model(project_file="",metabolic_model_name="",gui=True)
      ...
      def add_boundaries(self):
         ...
         reaction_selector,self.boundaries_reaction_selector_listbox=self.reaction_selector(labelframe_boundaries)
      ...
      def update_search(self):
          ...
          ###Reaction bounds
          search_term=self.search_entry.get()
          if self.active_search!=search_term:
      ...
      def add_flux_display(self):
         ...
         self.optimal_solution_listbox = Listbox(label_frame_determined_fluxes, yscrollcommand = scrollbar612.set, width=18 )
         ...
         self.fva_listbox = Listbox(frame_fva_list, yscrollcommand = scrollbar611.set ,width=32)
      ...
      def add_parameter(self):
          ...
          if selection_tupple!=(): 
             ...
             fva=flux_variability_analysis(model,fraction_of_optimum=self.fraction_of_optimum,reaction_list=[reaction_id],tolerance_feasibility=label_model.lp_tolerance_feasibility)
             ...
             if reaction_id==self.active_reaction_get_bounds:
                 self.boundaries_lb_spinbox.delete(0,"end")
                 self.boundaries_lb_spinbox.insert(INSERT,str(reaction.lower_bound))
                 self.boundaries_ub_spinbox.delete(0,"end")
                 self.boundaries_ub_spinbox.insert(INSERT,str(reaction.upper_bound))
      ...
      def export_fluxes(self):
           fileName = tkFileDialog.asksaveasfilename(parent=self.root,title="Save fluxes as...",filetypes=[("xlsx",".xlsx")])
           if len(fileName)>0:
      ...
      def export_sbml(self):
           fileName = tkFileDialog.asksaveasfilename(parent=self.root,title="Save model with constrains as...",filetypes=[("sbml",".sbml"),("xml",".xml")])
           if len(fileName)>0:
      ...
      def export_label(self):
           fileName = tkFileDialog.asksaveasfilename(parent=self.root,title="Save label simulation results as...",filetypes=[("xlsx",".xlsx")])
           if len(fileName)>0:
              if ".xlsx" not in fileName:
                     fileName+=".xlsx"
              a1,b=solver(self.label_model,mode="fsolve",fba_mode=self.fba_mode.get().lower())
      ...
      def add_turnover_as_parameter(self):
          turnover=self.selected_turnover
      ...
      def add_all_turnover_as_parameter(self):
          ...
          for turnover in self.label_model.turnover_flux_dict:
             value=self.label_model.turnover_flux_dict[turnover]
             self.label_model.parameter_dict[turnover+"_turnover"]={"v":self.label_model.turnover_flux_dict[turnover],"lb":0,"ub":max(value,turnover_upper_bound),"max_d":0.1,"type":"turnover","reactions":[turnover]} 
          apply_parameters(self.label_model,parameter_precision=self.parameter_precision,parameter_list=[turnover+"_turnover"])
      ...
      def update_label_sampling(self):
        a,b,c=get_objective_function(self.label_model,force_balance=self.label_model.force_balance,output=False,automatic_rsm=False)
        for condition in self.simulated_points_object_dict:
            base=60
            fig=self.condition_canvas_dict[condition]
            if self.label_window==True:
      ...
      def get_selected_reaction_name(self):
          ...
          if self.boundaries_reaction_selector_listbox.curselection()!=():
             ...
             print [reaction_id,position]
                ...
                print [reaction_id,position]
          elif self.turnover_selector_listbox.curselection()!=():
               position,=self.turnover_selector_listbox.curselection()
               reaction_id=self.turnover_selector_listbox.get(position)
               print [reaction_id,position]
