...
def cycle(cycle_arg_dict):
    ...
    for j in range(local_cycle_arg_dict["m"]):
        ...
        if (accept==True):
            ...
            if model.optimize(tolerance_feasibility=local_cycle_arg_dict["label_model"].lp_tolerance_feasibility).status!="optimal":
               error_dump(local_cycle_arg_dict["label_model"],"not optimal 1","accept")
               return
            error_dump(local_cycle_arg_dict["label_model"],"not optimal 1","currentparameters")
            #print label_model.flux_dict
    ...
    apply_parameters(local_cycle_arg_dict["label_model"], local_cycle_arg_dict["best_parameters"],apply_flux_values=True,parameter_precision=local_cycle_arg_dict["parameter_precision"],parameter_list=working_parameters_sample)
    f_best, parameter_dict,best_flux_dict=coordinate_descent(local_cycle_arg_dict["label_model"],parameter_dict=local_cycle_arg_dict["best_parameters"],parameter_to_be_fitted=local_cycle_arg_dict["parameter_to_be_fitted"],parameter_precision=local_cycle_arg_dict["parameter_precision"],perturbation=0.01,max_perturbation=local_cycle_arg_dict["max_perturbation"],force_flux_value_bounds=False,mode="fsolve",fba_mode="fba",debug=False,is_subprocess=True)
    local_cycle_arg_dict["f_best"]=f_best
    local_cycle_arg_dict["fc"]=f_best
    local_cycle_arg_dict["best_parameters"]=copy.deepcopy(parameter_dict)
    local_cycle_arg_dict["best_flux_dict"]=copy.deepcopy(best_flux_dict)
    cycle_arg_dict["current_parameters"]=copy.deepcopy(parameter_dict)
    return local_cycle_arg_dict
