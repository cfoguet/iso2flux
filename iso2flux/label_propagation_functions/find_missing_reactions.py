...
def find_missing_reactions(label_model,fn=None,fn_mode="a",verbose=True):
   ...
   for reaction in model.reactions:
       if reaction.id not in label_model.simplified_metabolic_model.reactions and (reaction not in ex_reactions) and (reaction.id not in label_model.reaction_merged_reactions_dict):
           ...
           if substrates!={} and products!={}:
              if "reflection" in reaction.notes:
                 if reaction.notes["reflection"] in missing_reactions_dict:
                     continue
              missing_reactions_dict[reaction.id]=[substrates.keys(),products.keys()]
   if fn!=None:
   ...
   for x in missing_reactions_dict: 
       ...
       if verbose:
          print "Warning: Reaction "+x+" lacks label propagation rules"
   if fn!=None:
